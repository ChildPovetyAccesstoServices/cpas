...
def validate_starts(locs, cs):
    ...
    for index, row in health.iterrows():
        ...
        if vals == vals:
            ...
            sq = [(i,j) for i in range(yInds-1, yInds+2) for j in range(xInds-1, xInds+2)]
...
if __name__ == "__main__":
    ...
    # find costs algorithm does not deal with np.NaN so change these to -9999 in cost surface
